# testchart-text-comparison

## Purpose

Automated pass/fail verification of text annotation rendering (caption anchor position against reference PNGs), extending the existing `compare_testchart.py` / `compare_thickness.py` testchart harness so text-annotation regressions are caught the same way stroke/colour regressions already are.
