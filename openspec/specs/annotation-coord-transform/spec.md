# annotation-coord-transform

## Purpose

Canonical, host-neutral OTIO-normalized annotation geometry (`otio_sync_core.coords`): the aspect-ratio scale, pixel↔OTIO-norm conversions, and shared annotation defaults that every host (RV, xStudio, the testchart generator) converts to and from. Host-specific unit conversions (RV's font/width scale factors, xStudio's font factor) live in each host's own codec, not here.
