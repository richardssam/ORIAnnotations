# xs-playlist-sequence-split

## Purpose

Defines how the xStudio playlist bin order and the sequence (timeline edit order) are represented and synchronised as separate concerns. xStudio has a distinct playlist bin (unordered media pool) and sequence timeline (edit order); RV has only an edit order. This spec governs how the two are communicated across the sync protocol.
