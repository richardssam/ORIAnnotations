# Capability: color-pipeline-sync

## Purpose
Carry OCIO color management in OTIO metadata — a timeline-level color group and a per-clip input colorspace — mirroring the OTIO Color Pipeline Model RFC verbatim, and synchronise it live across review peers over the existing transport without a new protocol message.
...
- **WHEN** the master changes a timeline color field that is in scope for sync
