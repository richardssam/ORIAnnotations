# Capability: color-pipeline-openrv

## Purpose
Apply synced color metadata to OpenRV's OCIO pipeline and write local input-color changes back into the metadata for broadcast. Input colorspace is synchronised; output (display) is explicitly out of scope.
