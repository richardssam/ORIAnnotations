# xstudio-live-stroke-broadcast

## Purpose

Defines xStudio's `AnnotationsCore` C++ plugin broadcast contract for in-progress ("live") pen strokes: the geometry-bearing 5-tuple sent to the `plugin_events_` group that lets a Python consumer (the ORI sync plugin) broadcast mid-stroke partials to peers directly from event data, without reading or scanning a bookmark.
