# Capability: color-pipeline-xstudio

## Purpose
Apply synced color metadata to xStudio's OCIO colour pipeline and write local input-color changes back into the metadata for broadcast. Input colorspace is synchronised; output (Display/View) is explicitly out of scope.
