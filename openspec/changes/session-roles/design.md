...
- Does the destructive-annotation row (`clear-all-paint` → driver only) need a category of its own, or is it a special case inside `annotation`? It is the only row where role is finer-grained than category, and `BROADCAST_CATEGORIES` maps whole methods rather than sub-operations. Handed here by `broadcast-ownership`, which declined to lease it.
