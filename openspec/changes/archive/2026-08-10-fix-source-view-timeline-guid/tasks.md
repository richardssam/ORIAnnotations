...
- [x] 3.5 Follow up on the two defects the verification surfaced — OpenRV broadcasting a stale `view_mode` during a view switch, and xStudio re-broadcasting a remote-induced `show_atom` as a local selection (design.md → Verification outcome). Raised as the `fix-view-switch-echo` change.
