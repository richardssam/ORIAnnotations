...
- **Join approval / admission control**: A session may want new joiners to be *approved by a driver before they see anything*, rather than merely landing on a restricted role. Deliberately not in this change, and not next: (1) it is a **receive/admission** control, while everything here is a send-side ceiling — it reopens both enforcement points this change rules out as non-goals; (2) it is not inert, so it forfeits the "zero behaviour change until a session opts in" property that is this change's whole risk story; (3) an approval **is** a role grant with an issuer, i.e. `session-role-administration`'s deferred `SET_PEER_ROLE` plus a fourth role below `viewer` and a joiner that waits — so it composes *after* that change rather than before it; and (4) it needs its own deadlock exit, structurally worse than D7's, since an unadmitted joiner in a session with no approver present can neither be approved nor self-elevate (D7 assumes an existing participant). Note also that the two readings of "see anything" separate cleanly: *"a new joiner must not be able to disrupt"* is already delivered by `default_role: viewer` once this change lands, whereas *"an unapproved person must not be in the session at all"* is broker access control — under the fanout topology an unadmitted peer already receives every message, so a role-layer gate can only ask its own client not to render, which is the cooperative-only guarantee design.md's last risk already flags. The second reading wants per-session credentials, a change with no dependency on roles. Prerequisites for the first are hedged in design.md D3 (reuse `peer_roles`, do not add a parallel allow-list) and D6 (a fourth role below `viewer` must not be reachable by absence).
