...
- [x] 5.4 Consider not scheduling a scan on `PaintPoint` — **closed as won't-fix**. Points still refresh the debounce timestamp, so a stroke whose points arrive slower than the 250 ms debounce schedules a scan per point. That only occurs with the synthetic probe (points 0.4 s apart); at real pointer rates the debounce absorbs every point and no extra scan happens, which the live capture confirms — 9 scans across a five-stroke session, all following a gesture boundary. Nothing is committed mid-stroke for such a scan to find, so the cost is wasted work in a case real drawing does not produce. Not worth another edit to the debounce path this change has just stabilised
...
- [x] 6.2 Establish whether the absence of a committed `Annotation.1` broadcast after pen-up is by design — **it is**. `Annotation.1 PARTIAL` is the mid-stroke channel only; the committed stroke goes out as `OTIO_SESSION_1.0 INSERT_CHILD` inserting a `Clip.2` named `Annotation_0` whose metadata carries `annotation_commands`, which is the pen-up path the plugin README documents. A real three-stroke session sent 7 `PARTIAL` and 4 `INSERT_CHILD`; no message is missing
...
- [x] 7.1 Run the annotation tests against the develop xStudio build and record which failures pre-date this change — full suite run twice plus a pre-change baseline. Final: **21 of 22 pass**. Every annotation test passes, including all six `openrv_draws_*` geometry tests and `xstudio_draws_pen_openrv_verifies`
- [x] 7.2 Regression found and fixed: restoring the 30 s interval broke `xstudio_draws_pen_openrv_verifies` (the harness writes bookmarks directly, so no draw event fires). Fixed in `649d4ab` by subscribing to the bookmarks actor's add/remove events, plus making a not-yet-resolvable bookmark retry instead of being dropped permanently — a pre-existing bug the 1 s scan had been masking
- [x] 7.3 Pre-existing failures separated from regressions: `add_media`, `delete_media_xstudio` and `reorder_media` are intermittent (fail in a suite run, pass in isolation or on re-run); `xstudio_selects` fails for an unrelated defect now scoped as the `sync-source-view-playback` change. Both recorded there as task 5.1
