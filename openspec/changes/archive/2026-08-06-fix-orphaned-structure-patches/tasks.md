...
- [x] 7.2 Run the full sync suite and confirm no regression in the RV↔RV structural tests (`delete_media_openrv`, `delete_media_openrv_noscript`, `otio_import_rv_to_rv`).
  ...
  - **Amendment.** The machine was under memory pressure during those runs, which was not known when the notes above were written. A clean re-run (22–35 GB free, zero swap across 99 samples) gives **23/24, with `add_media` the only `known_broken`** — every test named above passes, `delete_timeline` included, at a normal 60.7s and with no plugin silence. So the *conclusion* holds — none of these were caused by this change — but two of the stated *reasons* do not:
    - The `delete_timeline` "xStudio poll-thread stall" is unproven. It did not recur on a healthy machine, and memory pressure explains the 42s silence at least as well.
    - The media-materialisation fault was largely an artifact of the same pressure. `load_otio` measured 0.04s→1.23s under swap; healthy it is 0.01–0.51s with a **0.03s median**, against the harness's 1s command spacing. The window is real in code but ~30× smaller than the budget, and never coincided with an annotation in a healthy run.
  - The lesson worth carrying: environment is a variable in every one of these attributions, and none of the earlier runs recorded it. Suite runs now sample free memory and swap alongside the results so a compromised run is identifiable as such rather than mined for mechanisms that are not there.
