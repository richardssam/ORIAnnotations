...
- [x] 6.2 Consider a regression test for 5.3 specifically — "idle session broadcasts nothing" is cheap to assert, would have caught this immediately, and is the kind of property the spec's silence about convergence let slip — added `tests/xstudio_plugin/test_sequence_reconciliation_convergence.py` (moved out of `xstudio_plugin/tests/` — xStudio's plugin loader scans every subdirectory of `xstudio_plugin/` as a plugin candidate and logs a load failure for a non-plugin package there): mocks the xStudio-facing surface (no live session needed), asserts idle-broadcasts-nothing and that a direct-drag rebuild converges in one pass. Runs under xStudio's bundled Python (`xstudio_python`, see reference memory) via `pytest` or directly; verified to genuinely fail against the pre-fix single-track logic, not a vacuous pass
