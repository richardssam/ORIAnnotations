...
## Annotation deletion and visibility events share one event group -- but it never fires

`AnnotationsUI::send_event()` (the C++ source of `PaintClear`/`HideDrawings`/`ShowDrawings`/etc.) sends its `(event_atom, annotation_atom, JsonStore)` mail as a **direct point-to-point message to the AnnotationsCore actor**, not to AnnotationsUI's own event group. `on_annotation_event`, subscribed via `self.get_plugin("AnnotationsUI")` + `subscribe_to_plugin_events` (which joins `plugin_events_group_atom()`), was therefore **never going to see any of it** — confirmed live: zero occurrences across an entire test session, including ordinary stroke commits. (The pre-existing `"[TEST annotation_atom] Log every event..."` comment was already probing this same suspicion.) This is a pre-existing gap, not something any one change introduced — don't build new detection on `on_annotation_event` without verifying it actually fires first.

The working signal for a local clear is on the **other** subscription, `on_core_annotation_event` (`AnnotationsCore`'s `plugin_events_`, confirmed firing for live strokes as 4/5-tuples). `AnnotationsCore::clear_annotation()` broadcasts a **3-tuple** `(event_atom, annotation_data_atom, anno_ptr)` — one element shorter than the 4/5-tuple live-stroke shape — to the same group. A guard requiring `len(data) >= 4` silently swallows it. Accept `len(data) >= 3` and treat an exact 3-tuple as "something was cleared, go rescan" (no need to parse `anno_ptr` itself — the existing count-decrease detection below does the real work).

There is no working signal for `HideDrawings`/`ShowDrawings` at all — `AnnotationsCore::receive_annotation_data`'s branches for them just flip a local `hide_all_drawings_` bool with zero broadcast. Visibility sync instead relies entirely on the periodic poll (`poll_and_broadcast_display`, unconditional every 0.5s) reading `annotations_visible_`'s current value directly — no event needed for detection, just for lower latency, which isn't available here anyway.

Reading is via `self.plugin._ann_ui_plugin` — the `AnnotationsUI` plugin handle obtained with `self.get_plugin("AnnotationsUI")` at connect time and **retained** (the original code discarded it right after subscribing). `get_plugin()`'s `ModuleBase.__init__` eagerly queries `attribute_uuids_atom()` and wraps every existing remote attribute by uuid, so `ann_ui.get_attribute("Visibility").value()` works the same way `vp.get_attribute("Fit (F)")` does for the viewport.
